...
module StringEnums
  ...
  class_methods do
    def string_enum(enums_hash)
      ...
      define_method(enum_name.pluralize) do
        result = {}
      ...
      define_method(validation_method) do
        ...
        errors.add(enum_name, I18n.t("validations.invalid_#{enum_name}", :default => "Invalid value for #{enum_name}, possible values are: #{send(enum_name.pluralize).values}")) unless value.blank? or send(enum_name.pluralize).keys.include?(value.to_sym)
      end
    end
  end
