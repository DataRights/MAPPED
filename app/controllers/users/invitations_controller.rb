class Users::InvitationsController < Devise::InvitationsController
  ...
  def create
    if params[:customized_invitation]
      ...
      mail_content = params[:customized_invitation_content].gsub('$ACCEPT_INVITATION_URL$',Rails.application.routes.url_helpers.accept_user_invitation_url(:host => "#{request.protocol}#{request.host_with_port}", :invitation_token => user.raw_invitation_token))
