...
class TemplateTest < ActiveSupport::TestCase
  test "render exception handling" do
    assert_equal '', Template.new.render(nil)

    exception = assert_raise (RuntimeError ) {Template.new(content: 'This is a template').render(nil)}
    assert_equal 'Invalid context', exception.message

    exception = assert_raise (RuntimeError ) {Template.new(content: 'This is a template').render('wrong context')}
    assert_equal 'Wrong context type', exception.message

    assert_equal 'This is a template', Template.new(content: 'This is a template').render(TemplateContext.new)
  end
  ...
  test "render with simple context" do
    ...
    assert_equal render_result, Template.new(content: template_content).render(TemplateContext.new(user: User.new(email: 'test123@test.com'), organization: Organization.new(name: 'Schiphol Airport')))
  end
  ...
  test "render with context with organization with address" do
    ...
    organization.address = Address.new(line1: 'somewhere', city_name: 'London')
    assert_equal render_result, Template.new(content: template_content).render(TemplateContext.new(organization: organization))
  end
  ...
  test "render with context with user with address" do
    ...
    actual_render_result = Template.new(content: template_content).render(TemplateContext.new(user: user))
  ...
  test "render with context with user with first_name and last_name" do
    ...
    assert_equal render_result, Template.new(content: template_content).render(TemplateContext.new(user: user))
  end

  test 'language should be a symbol' do
		assert_equal :en, Template.new(content: 'This is a template',language: :en).language
	end
  ...
  test 'render with user, workflow, access request and organization' do
    ...
    assert_equal render_result, Template.new(content: template_content).render(TemplateContext.new(user: user, workflow: workflow, organization: organization, access_request: access_request))
  end
end
